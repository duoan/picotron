...
class DataParallelNaive(nn.Module):
    ...
    def __init__(self, module):
        ...
        self.require_backward_grad_sync = (
            True  # whether to synchronize gradients during backward pass. Set to False when using gradient accumulation
        )
...
class DataParallelBucket(nn.Module):
    ...
    def __init__(self, module, bucket_cap_mb=25, grad_type=torch.float32):
        ...
        self.bucket_manager = BucketManager(
            module.parameters(), pgm.process_group_manager.cp_dp_group, bucket_size, grad_type
        )
        ...
        for p in self.module.parameters():
            if p.requires_grad:
                p.grad = p.main_grad.to(
                    p.dtype
                )  # In PyTorch, you cannot assign a gradient with one data type to a tensor of another data type.
