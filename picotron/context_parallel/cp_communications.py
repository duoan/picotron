...
class ContextCommunicate:
    def __init__(self, msg: str = ""):
        ...
        if VERBOSE:
            print(
                f"RingComm ({msg}) | initialized | RANK:{self.rank} | "
                f"WORLD_SIZE:{self.world_size} | SEND_RANK:{self.send_rank} | "
                f"RECV_RANK:{self.recv_rank}",
                flush=True,
            )
    ...
    def send_recv(self, tensor_to_send, recv_tensor=None):
        ...
        if VERBOSE:
            print(
                f"RingComm | send_recv | STEP:{STEP} | RANK:{self.rank} | "
    ...
    def commit(self):
        ...
        if VERBOSE:
            print(
                f"RingComm | commit | STEP:{STEP} | RANK:{self.rank} | "
    ...
    def wait(self):
        ...
        for i, request in enumerate(self._active_requests):
            ...
            if VERBOSE:
                ...
                print(
                    f"RingComm | wait | STEP:{STEP} | RANK:{self.rank} | "
        ...
        if VERBOSE:
            print(f"RingComm | wait | STEP:{STEP} | RANK:{self.rank} | ACTION:all_operations_completed", flush=True)
